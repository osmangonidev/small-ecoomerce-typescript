...
export default function CartItem({ item, grossPrice, netPrice, cartItems, setCartItems}:any) {
  ...
  items.map((item:any) => {
    ...
    const grossSum = grossTotal.reduce((total:number, amount:number) => total + amount);
    const netSum = netTotal.reduce((total:number, amount:number) => total + amount);
    setGross(grossSum)
    setNet(netSum)
  }
